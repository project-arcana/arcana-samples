...
void phi_test::run_pbr_sample(phi::Backend& backend, sample_config const& sample_config, const phi::backend_config& backend_config)
{
    ...
    //
    {
        ...
        static_assert(true, "clang-format");
        {
            ...
            resources.mat_albedo = texgen_resources.load_texture(phi_test::sample_albedo_path, format::rgba8un, gc_enable_compute_mips, gc_enable_compute_mips);
            resources.mat_normal = texgen_resources.load_texture(phi_test::sample_normal_path, format::rgba8un, gc_enable_compute_mips, false);
            resources.mat_metallic = texgen_resources.load_texture(phi_test::sample_metallic_path, format::r8un, gc_enable_compute_mips, false);
            resources.mat_roughness = texgen_resources.load_texture(phi_test::sample_roughness_path, format::r8un, gc_enable_compute_mips, false);
        ...
        // transitions to SRV
        {
...

            {
                ...
                tcmd.add(resources.mat_albedo, resource_state::shader_resource, shader_domain::pixel);
                tcmd.add(resources.mat_normal, resource_state::shader_resource, shader_domain::pixel);
                tcmd.add(resources.mat_metallic, resource_state::shader_resource, shader_domain::pixel);
                tcmd.add(resources.mat_roughness, resource_state::shader_resource, shader_domain::pixel);
            ...
            {
                cmd::transition_resources tcmd;
                tcmd.add(resources.ibl_specular, resource_state::shader_resource, shader_domain::pixel);
                tcmd.add(resources.ibl_irradiance, resource_state::shader_resource, shader_domain::pixel);
                tcmd.add(resources.ibl_lut, resource_state::shader_resource, shader_domain::pixel);
                writer.add_command(tcmd);
            }
        ...
        if (!window.isMinimized())
        {
            ...
            cc::capped_vector<handle::command_list, 3> backbuffer_cmd_lists;
            {
...

                {
                    ...
                    cmd_trans.add(resources.colorbuffer_resolve, resource_state::shader_resource, shader_domain::pixel);
                    cmd_writer.add_command(cmd_trans);
                }
            ...
            // Data upload
            {
                ...
                std::memcpy(resources.current_frame().sb_modeldata_map, model_data, sizeof(phi_test::model_matrix_data));
