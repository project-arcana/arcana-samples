...
struct scene
{
    ...
    void addInstance(mesh const& mesh, material const& mat, tg::mat4 transform)
    {
...
struct asset_pack
{
    ...
    [[nodiscard]] mesh loadMesh(pr::Context& ctx, char const* path, bool binary = false);

    [[nodiscard]] material loadMaterial(pr::Context& ctx, pre::texture_processing& tex, char const* p_albedo, char const* p_normal, char const* p_arm);

private:
    cc::vector<inc::pre::pr_mesh> unique_meshes;
    cc::vector<pr::auto_texture> unique_textures;
    cc::vector<pr::auto_prebuilt_argument> unique_svs;
};
}
