...
// Results are saved into 2D g_lut_output texture in the form of DFG1 and DFG2 split-sum approximation terms,
...
RWTexture2D<float2> g_lut_output            : register(u0, space0);
...
[numthreads(32, 32, 1)]
void main_cs(uint2 ThreadID : SV_DispatchThreadID)
{
	// Get output g_lut_output dimensions.
	float outputWidth, outputHeight;
	g_lut_output.GetDimensions(outputWidth, outputHeight);
	...
	// We will now pre-integrate Cook-Torrance BRDF for a solid white environment and save results into a 2D g_lut_output.
	...
	g_lut_output[ThreadID] = float2(DFG1, DFG2) * InvNumSamples;
}
