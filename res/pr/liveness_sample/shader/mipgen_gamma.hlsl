...
void main_cs(uint2 thread_id : SV_DispatchThreadID)
{
	...
	float4 value0 = g_input.Load(sample_loc, int2(0, 0));
	float4 value1 = g_input.Load(sample_loc, int2(1, 0));
	float4 value2 = g_input.Load(sample_loc, int2(0, 1));
	float4 value3 = g_input.Load(sample_loc, int2(1, 1));
