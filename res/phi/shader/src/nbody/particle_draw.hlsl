...
static const float g_fParticleRad = 10.0f;
...
StructuredBuffer<PosVelo> g_bufPosVelo              : register(t0, space0);

ConstantBuffer<CameraConstants> g_camConstants      : register(b0, space0);
...
void GSParticleDraw(point VSParticleDrawOut input[1], inout TriangleStream<GSParticleDrawOut> SpriteStream)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        position = mul((float3x3)g_camConstants.invView, position) + input[0].pos;
        output.pos = mul(g_camConstants.worldViewProj, float4(position,1.0));
