...
struct specular_map_filter_settings
{
	// Roughness value to pre-filter for.
	float roughness;
};

[[vk::push_constant]] ConstantBuffer<specular_map_filter_settings> g_filter_settings          : register(b1, space0);

TextureCube g_input                                                     : register(t0, space0);
...
void main_cs(uint3 ThreadID : SV_DispatchThreadID)
{
	...
	g_input.GetDimensions(0, inputWidth, inputHeight, inputLevels);
	...
	for(uint i=0; i<NumSamples; ++i) {
		...
		float3 Lh = tangentToWorld(sampleGGX(u.x, u.y, g_filter_settings.roughness), N, S, T);
		...
		if(cosLi > 0.0) {
			...
			float pdf = ndfGGX(cosLh, g_filter_settings.roughness) * 0.25;
			...
			color  += g_input.SampleLevel(g_sampler, Li, mipLevel).rgb * cosLi;
			weight += cosLi;
		}
	}
	color /= weight;

	g_output[ThreadID] = float4(color, 1.0);
}
